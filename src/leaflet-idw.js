...
!(function () {
  ...
  (simpleidw.prototype = {
    ...
    discreteGradient: function (discreteGrad) {
      this._discreteGradient = discreteGrad;
      return this;
    },
    ...
    _colorize: function (pixels, gradient, opacity) {
      for (var i = 0, len = pixels.length, j; i < len; i += 4) {
        ...
        if (this._discrete) {
          const step = 1024 / this._discreteGradient.length;
          const color = this._discreteGradient[Math.floor(j / step)];
    ...
    */
    ...
    _initCanvas: function () {
      var canvas = (this._canvas = L.DomUtil.create(
        "canvas",
        "leaflet-idwmap-layer leaflet-layer"
      ));
    ...
    _updateOptions: function () {
      ...
      if (this.options.discreteGradient) {
        this._idw.discreteGradient(this.options.discreteGradient);
      }
      if (this.options.discrete) {
        this._idw.discrete(this.options.discrete);
      }
    },
